...
// With original app you need 24: 2x8 cards for first 2 phases, 2x4 cards for the middle 2 phases
var numCardsNeeded = 40

// This is the amount of cards found in the assets folder
// Change this if you add more cards
var numCardsToChooseFrom = 60
...
class CardModel{
    ...
    var A_locs = [3, 2, 8, 1, 7, 5, 4, 6, 9, 11, 10, 12]
    var Ap_locs = [3, 2, 8, 1, 7, 5, 4, 6, 9, 11, 10, 12, 13, 14, 15, 16, 17, 18, 19, 20]
    var B_locs = [3, 1, 8, 6, 4, 5, 2, 7, 11, 12, 10, 9]
    var Bp_locs = [3, 1, 8, 6, 4, 5, 2, 7, 11, 12, 10, 9, 13, 14, 15, 16, 17, 18, 19, 20]
    var C_locs = [6, 2, 7, 8, 4, 1, 5, 3, 9, 10, 12, 11]
    var Cp_locs = [6, 2, 7, 8, 4, 1, 5, 3, 9, 10, 12, 11, 13, 14, 15, 16, 17, 18, 19, 20]
    ...
    func initOrders() {
        order_A = [CGPoint(x: 191, y: 413), CGPoint(x: 1033, y: 142), CGPoint(x: 702, y: 871), CGPoint(x: 524, y: 169), CGPoint(x: 315, y: 837), CGPoint(x: 603, y: 571), CGPoint(x: 879, y: 344), CGPoint(x: 148, y: 879), CGPoint(x: 115, y: 132), CGPoint(x: 871, y: 743), CGPoint(x: 390, y: 475), CGPoint(x: 1082, y: 772)]
        
        // These are the flippable locations in the prime layout
        let Ap = [CGPoint(x:490, y:839), CGPoint(x:975, y:526), CGPoint(x:680, y:338), CGPoint(x:778, y:76), CGPoint(x:101, y:607), CGPoint(x:294, y:171), CGPoint(x:890, y:900), CGPoint(x:400, y:550)]
        
        order_A_prime = order_A + Ap

         order_B = [CGPoint(x: 922, y: 247), CGPoint(x: 249, y: 209), CGPoint(x: 934, y: 865), CGPoint(x: 756, y: 554), CGPoint(x: 83, y: 697), CGPoint(x: 326, y: 603), CGPoint(x: 514, y: 264), CGPoint(x: 532, y: 862), CGPoint(x: 1027, y: 520), CGPoint(x: 282, y: 844), CGPoint(x: 119, y: 480), CGPoint(x: 726, y: 150)]
        
        // These are the flippable locations in the prime layout
        let Bp = [CGPoint(x:370, y:351), CGPoint(x:709, y:367), CGPoint(x:777, y:884), CGPoint(x:92, y:863), CGPoint(x:531, y:533), CGPoint(x:78, y:84), CGPoint(x:900, y:500), CGPoint(x:600, y:700)]

        order_B_prime = order_B + Bp

        order_C = [CGPoint(x: 115, y: 888), CGPoint(x: 957, y: 246), CGPoint(x: 394, y: 710), CGPoint(x: 746, y: 773), CGPoint(x: 613, y: 424), CGPoint(x: 204, y: 187), CGPoint(x: 965, y: 485), CGPoint(x: 142, y: 507), CGPoint(x: 673, y: 155), CGPoint(x: 317, y: 414), CGPoint(x: 1007, y: 824), CGPoint(x: 568, y: 753)]
        
        // These are the flippable locations in the prime layout
        let Cp = [CGPoint(x:752, y:581), CGPoint(x:170, y:704), CGPoint(x:319, y:889), CGPoint(x:808, y:313), CGPoint(x:516, y:129), CGPoint(x:327, y:98), CGPoint(x:450, y:475), CGPoint(x:850, y:900)]
    ...
    func getMatchingCards(phase: Int) -> [Card] {
        ...
        if doneOnce == false && !unpausing {
            ...
            cardSets[0] += cardArray[0...11]
            cardSets[1] += cardArray[12...23]
            cardSets[2] += cardArray[24...31]
            cardSets[3] += cardArray[32...39]
    ...
    func getTargetCards(phase: Int) -> [Card] {
        ...
        if phase == 1 || phase == 5 {
            tan += cardNumArray[0...11]
        } else if phase == 2 || phase == 6 {
            tan += cardNumArray[12...23]
        } else if phase == 3 {
            tan += cardNumArray[24...31]
        } else if phase == 4 {
            tan += cardNumArray[32...39]
        }
